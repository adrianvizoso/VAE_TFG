...
checkpoint_dir <- "./checkpoints_mmd_vae"
...
TrainModel<- function(encoder, 
  ...
  generator(epoch = 0,
            ...
            filename = "mmd_vae_generated_epoch_",
            latent_dim = latent_dim)
  
  if(latent_dim==2) plotLatentSpace(epoch = 0,
                                    MMD = T,
                                    filename = 'mmd_vae_latent_space_epoch_')
  plotGrid(epoch = 0,
           latent_dim = latent_dim,
           filename = 'mmd_vae_grid_epoch_')
  ...
  for (epoch in seq_len(num_epochs)) {
    ...
    if (epoch %% 10 == 0) {
      ...
      if (latent_dim==2) plotLatentSpace(epoch = epoch,
                                         MMD = T, 
                                         filename = "mmd_vae_latent_space_epoch_")
